...
ui <- fluidPage(
    ...
    # Application title
    titlePanel("CoViD-19 data exploration \n "),
    ...
    sidebarLayout(
        ...
        mainPanel(
            tabsetPanel(
                # tabPanel("waffle plot", plotOutput("PlotWaffleCountry", width = "537px", height = "650px")),
                tabPanel("linear plot", plotOutput("PlotCasesLin", width = "1100px", height = "850px"))
                # tabPanel("logarithmic plot", plotOutput("PlotCasesLog", width = "1100px", height = "850px")),
                # tabPanel("changes plot", plotOutput("PlotCasesDiffLin", width = "1100px", height = "850px"),
...
server <- function(input, output) {
    ...
    # # For waffle plot (for selected country).
    # dd_waffle_country <-  eventReactive(input$show_button, {
    #     dd_plus %>% 
    #         filter(., country_region %in% input$country & date == input$date) %>%
    #         mutate(., status = fct_relevel(status, "confirmed", "active", "recovered", "deaths")) %>% 
    #         filter(., status != "confirmed") %>% 
    #         group_by(., country_region) %>% 
    #         mutate(., cases = round_preserve(cases/sum(cases_all)*100),
    #                image = list(img)) %>% 
    #         ungroup() 
    # })
    ...
    # type <- eventReactive(input$show_button, {
    #     dplyr::if_else(input$check_type == "absolute values", "absolute", "relative")
    # })
    ...
    # -------------------------------------------------------   
    # PLOT 1: Aggregated case numbers -----------------------
    
    p01_PlotCasesLin <- eventReactive(input$show_button, {
        if (input$check_type == "absolute values") {
            ggplot() +
    ...
    output$PlotCasesLin <- renderPlot({ p01_PlotCasesLin() })
    ...
    # output$downloadPlot <- downloadHandler(
    #     filename = function() {
    #         paste("CoViD-19-AggrCasesLin", "png", sep=".")
    #     },
    #     content = function(file){
    #         ggsave(file, plot = )
    #     }
    ...
    output$PlotCasesLog <- renderPlot({
        if (input$check_type == "absolute values") {
            ggplot() +
                geom_line(data = dd_base_viz(),
                          aes(x = time_ind, y = cases_all, group = as.factor(country_region),
    ...
    output$PlotCasesDiffLin <- renderPlot({
        if (input$time_cat == "days") {
        ...
        iron(
            waffle(dd_waffle_country_vec(), 
                ...
                theme(text = element_text(family = "SF Pro Rounded"),
                      ...
                      plot.caption = element_text(size = 8, hjust = 0.05, vjust = -0.1, colour = "#7F8182", margin = margin(10,0,10,0)))
        )
    })
